...
class PayrollDateMonthlyWiseServletTest {

	private PayrollDateMonthlyWiseServlet servlet;
	...
	void setUp() throws Exception {
		servlet = new PayrollDateMonthlyWiseServlet();
	...
	@Test
	void testDoGet_WithValidMonth_ShouldRenderMonthlyPayroll() throws Exception {
		when(request.getParameter("month")).thenReturn("2025-11");

		PayrollStatement ps = new PayrollStatement();
		when(payrollServiceMock.getPayrollsMonthWise("2025-11")).thenReturn(Arrays.asList(ps));

		// Mock Thymeleaf rendering to avoid actual HTML processing
		doAnswer(inv -> {
			PrintWriter writer = inv.getArgument(2);
			writer.write("Rendered Monthly Payroll");
			return null;
		}).when(templateEngineMock).process(anyString(), any(WebContext.class), any(PrintWriter.class));

		servlet.doGet(request, response);

		verify(templateEngineMock).process(eq("monthlywisepayroll"), any(WebContext.class), any(PrintWriter.class));
		String output = responseWriter.toString();
		assert output.contains("Rendered Monthly Payroll");
	}

	@Test
	void testDoGet_WithNoMonth_ShouldRenderNoDataFound() throws Exception {
		...
		verify(templateEngineMock).process(eq("monthlywisepayroll"), any(WebContext.class), any(PrintWriter.class));
		assert responseWriter.toString().contains("No data found");
	}
	...
	void testDoPost_DelegatesToDoGet() throws Exception {
		...
		doAnswer(inv -> {
			PrintWriter writer = inv.getArgument(2);
			writer.write("Post Delegation Success");
			return null;
		}).when(templateEngineMock).process(anyString(), any(WebContext.class), any(PrintWriter.class));

		servlet.doPost(request, response);

		verify(templateEngineMock, times(1)).process(eq("monthlywisepayroll"), any(WebContext.class),
				any(PrintWriter.class));
		assert responseWriter.toString().contains("Post Delegation Success");
	}
}
